...
class ReportTestResult(unittest.TestResult):
    ...
    def stopTestRun(self, title=None) -> dict:
        ...
        # print(json.dumps(self.fields, indent=4).encode().decode('unicode-escape'))
        global result
        result.clear()
        # result.append(len(self.result_list))
        # result.append(self.success_counter)
        # result.append(self.failure_count)
        # result.append(self.skipped)
        result.append(self.fields['testAll'])
        result.append(self.fields['testPass'])
        result.append(self.fields['testFail'])
        data["total"] = self.fields['testAll']
        data['pass'] = self.fields['testPass']
        data['fail'] = self.fields['testFail']
        # result.append(self.fields['testSkip'])
        # print(self.fields)
        return self.fields
    ...
    def addSuccess(self, test) -> None:
        ...
        if self.verbosity > 1:
            ...
            fun_name = '{}'.format(test)
            test_name = fun_name.split('(', 1)[0]
            sys.stderr.write(
                '第{}个用例执行完成    P   '.format(
                    self.num) +
                '{}'.format(test_name) +
    ...
    def addFailure(self, test, err):
        ...
        self._mirrorOutput = True
        # global fail_count
        # fail_count.append(self.failure_count)
    ...
    def report(
        ...
        if log_path:
            ...
            message = (
                ...
                "e.g. result.report(filename='测试报告_demo', description='自动化测试报告', report_dir='report')")
